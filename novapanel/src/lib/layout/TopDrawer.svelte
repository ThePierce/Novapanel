...
<div class="drawer-shell" class:open={controlsOpen} class:showHint={showDrawerHint && !controlsOpen}>
	...
	<button
		...
		aria-label={controlsOpen ? t('closeDrawer') : t('openDrawer')}
	>
...
<style>
	...
	.drawer-shell.showHint::before {
		opacity: 0.72;
	}
	.drawer-shell.showHint::after {
		opacity: 0.72;
		transform: translateX(0.52rem) rotate(45deg);
	}
	.top-drawer {
	...
	.edit-mode-inline {
		padding: 0.65rem 1rem 0.4rem 0.82rem;
		border-radius: 0.4rem;
	...
	.drawer-shell:has(.drawer-toggle:hover) .drawer-toggle {
		opacity: 1;
	}
</style>
