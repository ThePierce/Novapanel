<script lang="ts">
	...
	import { coerceCurrencyCode, currencyOptions, DEFAULT_CURRENCY_CODE } from '$lib/currency';
	...
	type Props = {
		...
		onSetCurrencyCode: (value: string) => void;
	...
	let spotifyAuthStarting = $state(false);
	let spotifyAuthCheckPromise: Promise<void> | null = null;
	let spotifyAuthCheckSeq = 0;
	let spotifyAuthCheckTimer: ReturnType<typeof setTimeout> | null = null;
	...
	const exampleRedirectUri =
		'https://jouw-home-assistant.example/api/hassio_ingress/<token>/api/spotify/auth/callback';
	...
	function clearSpotifyAuthCheckTimer() {
		if (spotifyAuthCheckTimer) {
			clearTimeout(spotifyAuthCheckTimer);
			spotifyAuthCheckTimer = null;
		}
	}

	function scheduleSpotifyAuthCheck(delayMs = 400) {
		clearSpotifyAuthCheckTimer();
		spotifyAuthCheckTimer = setTimeout(() => {
			spotifyAuthCheckTimer = null;
			void checkSpotifyAuth();
		}, delayMs);
	}

	async function checkSpotifyAuth({ force = false }: { force?: boolean } = {}) {
		if (spotifyAuthCheckPromise && !force) return spotifyAuthCheckPromise;
		const seq = ++spotifyAuthCheckSeq;
		spotifyAuthStatus = 'checking';
		spotifyAuthError = '';
		const request = (async () => {
			try {
				const resp = await fetchWithTimeout(buildIngressPath('/api/spotify/auth/status'), {}, 8000);
				if (seq !== spotifyAuthCheckSeq) return;
				if (!resp.ok) {
		...
		spotifyAuthCheckPromise = request;
		try {
			await request;
		} finally {
			if (spotifyAuthCheckPromise === request) spotifyAuthCheckPromise = null;
		}
	}
	...
	function handleSpotifyAuthMessage(event: MessageEvent) {
		...
		if (
			...
			stopAuthPolling();
			void checkSpotifyAuth({ force: true });
		}
	}
	...
	$effect(() => {
		...
		return () => {
			...
			clearSpotifyAuthCheckTimer();
			spotifyAuthCheckSeq += 1;
			stopAuthPolling();
		};
	});

	async function startSpotifyAuth() {
		if (spotifyAuthStarting || spotifyAuthInProgress) return;
		// Eerst credentials opslaan, daarna een nieuw tabblad openen met de auth-URL.
		spotifyAuthError = '';
		spotifyAuthStarting = true;
		try {
			if (shouldUseDetectedSpotifyRedirectUri(spotifyRedirectUri)) {
				onSetSpotifyRedirectUri(detectedRedirectUri);
			}
			await Promise.resolve(onSave());
			const resp = await fetchWithTimeout(buildIngressPath('/api/spotify/auth/start'), {}, 10000);
			if (!resp.ok) {
				const text = await resp.text();
				spotifyAuthError =
					`${translate('Kon Spotify-auth niet starten. Controleer de credentials en redirect URI.', selectedLanguage)} (${resp.status}). ${text || ''}`.trim();
				return;
			}
			const data = await resp.json();
			if (!data?.url) {
	...
	$effect(() => {
		...
		scheduleSpotifyAuthCheck();
		return clearSpotifyAuthCheckTimer;
	});
...
<div
	class="settings-modal app-popup settings-config-modal np-detail"
	role="dialog"
	aria-modal="true"
	...
	{#if activeSettingsTab === 'general'}
		<div class="np-detail-body settings-body">
			<div class="settings-section">
				...
				<LanguageConfig {selectedLanguage} label={t('language')} onLanguageChange={onSetLanguage} />
				<label class="settings-field" for="np-currency-code">
					<span>{translate('Valuta', selectedLanguage)}</span>
					<select
						id="np-currency-code"
						value={coerceCurrencyCode(selectedCurrencyCode, DEFAULT_CURRENCY_CODE)}
						onchange={(event) => onSetCurrencyCode((event.currentTarget as HTMLSelectElement).value)}
					>
					...
					<form class="integration-fields" onsubmit={(event) => event.preventDefault()}>
						...
						<div class="integration-help-text">
							...
							<a
								...
								target="_blank"
								rel="noopener">developer.spotify.com/dashboard</a
							>.
							{translate(
						...
						/>
						<div class="integration-help-text">
							{translate('Open je app op', selectedLanguage)}
							<a
								class="integration-help-link"
								href="https://developer.spotify.com/dashboard"
								target="_blank"
						...
						<div class="integration-actions">
							<button
								type="button"
								class="np-btn"
								onclick={() => void copyDetectedRedirect()}
								disabled={!detectedRedirectUri}
							>
								<TablerIcon name={copiedRedirect ? 'check' : 'copy'} size={14} />
								{copiedRedirect
									? translate('Gekopieerd', selectedLanguage)
									: translate('Kopieer mijn callback-URL', selectedLanguage)}
							</button>
						</div>
						...
						<div class="integration-actions">
							...
							{:else if spotifyAuthInProgress || spotifyAuthStarting}
								<button type="button" class="np-btn primary spotify" disabled>
									<TablerIcon name="loader-2" size={14} />
									{spotifyAuthStarting
										? translate('Verbinding starten…', selectedLanguage)
							...
							<button
								type="button"
								class="np-btn"
								disabled={spotifyAuthStatus === 'checking'}
								onclick={() => void checkSpotifyAuth({ force: true })}
							>
...
<style>
	...
	.np-detail-body::-webkit-scrollbar {
		width: 0;
		height: 0;
		display: none;
	}
	...
	.integration-badge.warn {
		...
		color: #fbbf24;
		border-color: rgba(243, 202, 98, 0.3);
	}
	.integration-chevron {
		display: inline-flex;
		align-items: center;
	...
	.integration-fields label:first-of-type {
		margin-top: 0;
	}
	...
	.integration-fields input:hover {
	...
	.integration-status.off {
		color: #fbbf24;
		background: rgba(243, 202, 98, 0.1);
		border-color: rgba(243, 202, 98, 0.3);
	}
	...
	.column-controls button.active .col-pick-vis i {
		opacity: 0.85;
	}
