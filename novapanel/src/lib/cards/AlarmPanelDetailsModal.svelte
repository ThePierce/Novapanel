<script lang="ts">
	...
	const alarmState = $derived((alarmEntity?.state ?? '').toLowerCase());
	const tone = $derived(
		alarmState.includes('disarmed')
			? 'safe'
			: alarmState.includes('arming') || alarmState.includes('pending')
				? 'pending'
				: alarmState.includes('triggered')
					? 'triggered'
					: 'armed'
	);
	...
	const isDisarmed = $derived(alarmState.includes('disarmed'));
	const modes = [
		{
			label: translate('Thuis', $selectedLanguageStore),
			service: 'alarm_arm_home',
			icon: 'mdi:shield-home-outline'
		},
		{
			label: translate('Weg', $selectedLanguageStore),
			service: 'alarm_arm_away',
			icon: 'mdi:shield-lock-outline'
		},
		{
			label: translate('Nacht', $selectedLanguageStore),
...
<div
	class="alarm-modal np-detail"
	role="dialog"
	aria-modal="true"
	aria-label={t('cardTypeAlarmPanel')}
	...
	{:else}
		<div class={`alarm-body tone-${tone}`}>
			...
			{#if !isDisarmed}
				...
				<div class="numpad">
					{#each ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', '⌫'] as key (key || 'spacer')}
						{#if key === ''}<div></div>
						{:else if key === '⌫'}
							<button
								type="button"
								class="numpad-btn numpad-back"
								onclick={tapBack}
								disabled={busy}
								aria-label={translate('Wis', $selectedLanguageStore)}
							>
								<svg
									width="22"
									height="22"
						...
						{:else}
							<button type="button" class="numpad-btn" onclick={() => tapDigit(key)} disabled={busy}
								>{key}</button
							>
						{/if}
					{/each}
			...
			<div class="alarm-actions">
				{#if isDisarmed}
					{#each modes as mode (mode.service)}
				...
				{:else}
					<button
						type="button"
						class="action-btn action-disarm"
						onclick={() => callAlarm('alarm_disarm')}
						disabled={busy}
					>
						<StatusIcon icon="mdi:shield-off-outline" size={18} /><span
							>{translate('Uitschakelen', $selectedLanguageStore)}</span
						>
					</button>
				{/if}
			</div>
...
<style>
	...
	.tone-safe {
		--np-tone: #4ade80;
		--np-tone-glow: rgba(74, 222, 128, 0.32);
	}
	.tone-armed {
		--np-tone: #f87171;
		--np-tone-glow: rgba(248, 113, 113, 0.32);
	}
	.tone-pending {
		--np-tone: #fbbf24;
		--np-tone-glow: rgba(251, 191, 36, 0.32);
	}
	.tone-triggered {
		--np-tone: #ff4444;
		--np-tone-glow: rgba(255, 68, 68, 0.42);
	}
	...
	.tone-triggered .alarm-bg-glow {
		animation: bg-pulse 0.9s ease-in-out infinite;
	}
	@keyframes bg-pulse {
		0%,
		100% {
	...
	.ring-pulse {
		stroke: var(--np-tone);
		animation: ring-pulse-anim 1.4s ease-out infinite;
	}
	...
	.hero-icon-bg {
		...
		place-items: center;
		background:
			radial-gradient(circle at 30% 30%, rgba(255, 255, 255, 0.05), transparent 70%), rgba(0, 0, 0, 0.3);
		border: 0.5px solid rgba(255, 255, 255, 0.08);
		color: var(--np-tone);
		box-shadow:
			inset 0 0 24px rgba(0, 0, 0, 0.4),
			0 0 36px var(--np-tone-glow);
		transition:
			box-shadow 0.5s,
			color 0.5s;
		position: relative;
	...
	.action-disarm {
		background:
			linear-gradient(135deg, rgba(248, 113, 113, 0.14), transparent 60%), rgba(248, 113, 113, 0.08);
		color: #fca5a5;
		border-color: rgba(248, 113, 113, 0.25);
	}
	.action-disarm:hover:not(:disabled) {
		background:
			linear-gradient(135deg, rgba(248, 113, 113, 0.22), transparent 60%), rgba(248, 113, 113, 0.12);
