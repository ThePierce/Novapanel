<script lang="ts">
	...
	function uvLabel(uv: number): { text: string; color: string } {
		if (uv <= 2) return { text: translate('Laag', activeLanguage), color: '#4caf50' };
		if (uv <= 5) return { text: translate('Matig', activeLanguage), color: '#f9a825' };
		if (uv <= 7) return { text: translate('Hoog', activeLanguage), color: '#f57c00' };
		if (uv <= 10) return { text: translate('Zeer hoog', activeLanguage), color: '#d32f2f' };
		return { text: translate('Extreem', activeLanguage), color: '#9c27b0' };
	}
	...
	function formatHour(dt: string): string {
		if (!dt) return '-';
		try {
			return new Date(dt).toLocaleTimeString(activeLocale, { hour: '2-digit', minute: '2-digit' });
		} catch {
			return '-';
		}
	}
	...
	const feelsLikeNow = $derived(
		typeof attrs.apparent_temperature === 'number' ? Math.round(attrs.apparent_temperature) : null
	);
	...
	const tempCurve = $derived.by(
		(): {
			...
			const areaPath = path
				? `${path} L ${points[points.length - 1].x.toFixed(1)} ${(H - padBottom).toFixed(1)} L ${points[0].x.toFixed(1)} ${(H - padBottom).toFixed(1)} Z`
				: '';
			const precipMax = Math.max(
				0.5,
				...items.map((i) => (typeof i.precipitation === 'number' ? i.precipitation : 0))
			);
...
<div
	class="settings-modal app-popup weather-detail-modal np-detail"
	role="dialog"
	aria-modal="true"
	aria-label={t('cardTypeWeather')}
...
>
	...
	<div class="weather-body">
		...
		<div class="weather-hero">
			<div class="hero-cond">
				{#if condition}
					...
					<img
						class="hero-icon"
						src={heroSrc}
						alt={translateCondition(condition)}
						width="78"
						height="78"
			...
			<div class="hero-text">
				...
				{#if feelsLikeNow !== null && feelsLikeNow !== currentTemp}
					<div class="hero-feels">{translate('Voelt als', activeLanguage)} {feelsLikeNow}{tempUnit}</div>
				{/if}
			</div>
			<div class="hero-sun">
				{#if sunriseTime}
					<div class="hero-sun-item">
						<svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
							...
							<path
								d="M12 4v1.5M5.6 5.6l1.1 1.1M4 12h1.5M5.6 18.4l1.1-1.1M12 18.5V20M18.4 18.4l-1.1-1.1M20 12h-1.5M18.4 5.6l-1.1 1.1"
								stroke="#ffd35a"
								stroke-width="1.5"
		...
		{#if Object.keys(attrs).length > 0}
			<div class="metrics-bar">
				...
				{#if uvInfo && uvIndex !== null}
					<span class="chip" style="color:{uvInfo.color}"
						>☀️ UV {uvIndex.toLocaleString(activeLocale, {
				...
				{#if typeof attrs.pressure === 'number'}
					<span class="chip"
						>🔵 {Math.round(attrs.pressure)}
						{typeof attrs.pressure_unit === 'string' ? attrs.pressure_unit : 'hPa'}</span
					>
				{/if}
				...
				{#if typeof attrs.wind_speed === 'number'}
					<span class="chip">
						...
						{#if typeof attrs.wind_bearing === 'number'}
							{bearingToCompass(attrs.wind_bearing as number)}{toBeaufort(
								attrs.wind_speed as number,
								attrs.wind_speed_unit
							)}
						{:else}
							{Math.round(attrs.wind_speed as number)}
							{typeof attrs.wind_speed_unit === 'string' ? attrs.wind_speed_unit : 'km/h'}
						{/if}
					</span>
				{/if}
			</div>
			...
			<div class="chart-tile">
				<div class="chart-head">
					<span class="chart-title"
						>{translate('Temperatuur', activeLanguage)} — {translate('komende 24u', activeLanguage)}</span
					>
					<span class="chart-range"
						>{Math.round(tempCurve.minT)}{tempUnit} — {Math.round(tempCurve.maxT)}{tempUnit}</span
					>
				</div>
				<svg
					class="temp-chart"
					...
					{#each tempCurve.points as p, i (p.idx)}
						{#if i % 6 === 0 || i === tempCurve.points.length - 1}
							<text
								...
								font-size="13"
								font-weight="500">{Math.round(p.temp)}°</text
							>
						{/if}
					{/each}
				</svg>
		...
		{#if next24Hours.length > 0}
			...
			<div class="hour-strip">
				{#each next24Hours as item, idx (idx)}
					...
					<div class="hour-card">
						<div class="hour-time">
							{idx === 0 ? translate('Nu', activeLanguage) : formatHour(dt)}
						</div>
						<div class="hour-icon">
							{#if !hourlyIconFailed[idx] && !skipMc && cond}
								<img
									{src}
									alt={translateCondition(cond)}
									width="36"
									height="36"
						...
						{#if speed !== null}
							<div class="hour-wind">
								{#if bearing !== null}
									...
									<span>{bearingToCompass(bearing)}{toBeaufort(speed, item.wind_speed_unit)}</span>
								{:else}
		...
		{:else}
			<div class="empty-state">{translate('Uurvoorspelling laden…', activeLanguage)}</div>
		{/if}
	</div>
</div>
...
<style>
	...
	.weather-hero {
		...
		padding: 16px 18px;
		background:
			linear-gradient(135deg, rgba(34, 211, 238, 0.06), transparent 60%), rgba(255, 255, 255, 0.025);
		border: 0.5px solid rgba(255, 255, 255, 0.08);
		border-radius: 13px;
	}
	.hero-cond {
