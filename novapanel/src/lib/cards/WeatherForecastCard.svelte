<script lang="ts">
	...
	import { subscribeWeatherForecastDirect, type WeatherForecastType } from '$lib/ha/weather-forecast-service';
	...
	function dateWithReferenceTime(day: Date, reference: Date): Date {
	...
	function isForecastNight(item: Record<string, unknown>): boolean {
		if (forecastType === 'daily') return false;
		const datetime = parseDate(item.datetime);
		if (!datetime)
			return String(item.condition ?? '')
				.toLowerCase()
				.includes('night');
		const sunrise = parseDate(storeSun?.attributes?.next_rising);
		const sunset = parseDate(storeSun?.attributes?.next_setting);
		if (sunrise && sunset) {
			const sunriseOnDay = dateWithReferenceTime(datetime, sunrise);
			const sunsetOnDay = dateWithReferenceTime(datetime, sunset);
			if (sunriseOnDay < sunsetOnDay) {
				return datetime < sunriseOnDay || datetime >= sunsetOnDay;
			}
			return datetime >= sunsetOnDay && datetime < sunriseOnDay;
		}
...
{:else}
	<div class="container">
		{#each shown as item, idx (idx)}
			...
			{@const night = isForecastNight(item)}
			{@const src = `${ingressBase}/weather/meteocons/${meteoconName(cond, night)}.svg`}
			<div class="item">
				...
				<div class="icon" title={cond ? tCondition(cond) : ''}>
					{#if !iconFailed[idx]}
						...
						/>
					{:else}
						<WeatherIcon condition={cond} {night} size={34} />
					{/if}
				</div>
