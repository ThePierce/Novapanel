<script lang="ts">
	...
	import {
		...
		readMediaTitle,
		readMuted,
		readNowPlayingSummary,
		readSources,
		readVolume,
		richestEntityFor,
		spotifyUriToMaMediaType,
		type MaTarget,
		type OnkyoBridge,
		type SpUri,
		type TuneInFavorite,
		type TuneInItem
	} from '$lib/cards/status/media-hub-utils';
	...
	function saveTuneInFavs(list: TuneInFavorite[]) {
		try {
			writeStoredValue(TUNEIN_FAVS_KEY, JSON.stringify(list));
		} catch {}
	}
	...
	$effect(() => {
		...
		const onkyoZones = sortedEntities.filter((e) => {
			...
			return (
				lower.includes('onkyo') ||
				lower.includes('tx_nr') ||
				lower.includes('tx-nr') ||
		...
		const initial: OnkyoBridge[] = onkyoZones.map((entity) => {
			...
			if (!/woonkamer|keuken|zone/i.test(label)) {
				label =
					entity.entityId.includes('zone_2') || entity.entityId.includes('zone2')
						? `Onkyo Zone 2`
						: `Onkyo Hoofdzone`;
			}
			return {
	...
	const maRadioHubs = $derived.by(() => {
		...
		if (onkyoBridges.length > 0) {
			...
			for (const bridge of onkyoBridges) {
				onkyoMaId = findMaEntityForBase(bridge.zoneEntityId, all);
				if (onkyoMaId) break;
			}
			if (onkyoMaId) {
		...
		for (const e of all) {
			...
			const ma = findMaEntityForBase(e.entityId, all);
	 ...
	 */
	async function tryOnkyoTransport(
	...
	async function toggleMute(entity: HomeAssistantEntity) {
		await callService('media_player', 'volume_mute', entity.entityId, {
			is_volume_muted: !readMuted(entity)
		});
	}
	...
	function handleSpotifyAuthMessage(event: MessageEvent) {
		...
		if (
			data &&
			typeof data === 'object' &&
			data.type === 'novapanel-spotify-auth' &&
	...
	function findOnkyoSpotifySource(entityId: string): string | null {
		...
		const isOnkyo =
			entityId.toLowerCase().includes('onkyo') ||
	...
	$effect(() => {
		...
		const isBridgeActive =
			activeEntity !== null && onkyoBridges.some((b) => b.zoneEntityId === activeEntity.entityId);
	...
	async function searchTuneIn() {
		...
		try {
			const resp = await fetchWithTimeout(
				ingressPath(`/api/tunein/search?q=${encodeURIComponent(q)}`),
				{},
				12000
			);
			if (!resp.ok) {
	...
	async function playTuneInItem(item: TuneInItem) {
		try {
			...
			if (/Tune\.ashx|\.pls(\?|$)|\.m3u(\?|$)/i.test(item.url)) {
				const resp = await fetchWithTimeout(
					ingressPath(`/api/tunein/resolve?url=${encodeURIComponent(item.url)}`),
					{},
					12000
				);
				if (resp.ok) {
			...
			if (tuneInPlaybackTarget.startsWith('ma:')) {
				...
				const target = maRadioHubs.find(
					(h) => h.baseEntityId === baseEntityId && h.maEntityId === maEntityId
				);
				if (!target) {
				...
				if (isOnkyoTarget) {
					...
					try {
						...
						if (
							onkyoDevice &&
							(spotifyPlayerState.isPlaying || spotifyPlayerState.device?.id === onkyoDevice.id)
						) {
							await spJson('/api/spotify/control', {
								method: 'POST',
								headers: { 'content-type': 'application/json' },
				...
				if (item.image || item.text) {
					radioImageByBase = {
						...radioImageByBase,
						[baseEntityId]: { image: item.image ?? '', name: item.text ?? '' }
					};
				}
				return;
			}
...
<div class="media-hub">
	...
	<section class="hub-now">
		{#if activeEntity}
			...
			{@const radioMaTargetMatchesActive =
				tuneInPlaybackTarget.startsWith('ma:') &&
				tuneInPlaybackTarget.slice('ma:'.length).split('|')[0] === activeEntity.entityId}
			{@const spotifyMaTargetMatchesActive =
				spotifyPlaybackTarget.startsWith('ma:') &&
			...
			{@const mediaEntity = useBridge ? activeEntity : richestEntityFor(activeEntity, entities, allEntities)}
			...
			{@const rawCover = useBridge
				? (spotifyPlayerState.track?.image ?? '')
				: readMediaImage(mediaEntity) ||
					(maIsRadio ? (radioImageByBase[activeEntity.entityId]?.image ?? '') : '')}
			{@const cover = displayImageUrl(rawCover)}
			{@const title = useBridge
				? (spotifyPlayerState.track?.name ?? '')
				: readMediaTitle(mediaEntity) ||
					(maIsRadio ? (radioImageByBase[activeEntity.entityId]?.name ?? '') : '')}
			...
			{@const mediaPos =
				typeof maAttrs.media_position === 'number' ? (maAttrs.media_position as number) : null}
			{@const mediaDur =
				typeof maAttrs.media_duration === 'number' ? (maAttrs.media_duration as number) : null}
			{@const spotifyProgressMs =
				typeof spotifyPlayerState.progressMs === 'number' ? spotifyPlayerState.progressMs : null}
			{@const spotifyDurationMs =
				typeof spotifyPlayerState.track?.durationMs === 'number' ? spotifyPlayerState.track.durationMs : null}
			{@const progressPositionMs = mediaPos !== null ? mediaPos * 1000 : useBridge ? spotifyProgressMs : null}
			{@const progressDurationMs = mediaDur !== null ? mediaDur * 1000 : useBridge ? spotifyDurationMs : null}
			{@const progressPct =
				progressPositionMs !== null && progressDurationMs && progressDurationMs > 0
					? Math.min(100, (progressPositionMs / progressDurationMs) * 100)
					: null}
			...
			<div class="now-hero-content">
				...
				<div class="now-meta">
					<div class="now-device">
						...
						<span
							>{getNowPlayingDeviceLabel(
								activeEntity.entityId,
								activeEntity.friendlyName ?? activeEntity.entityId
							)}</span
						>
					</div>
					...
					{#if progressPct !== null}
						<div class="now-progress">
							...
							<div class="now-progress-times">
								<span>{formatMs(progressPositionMs ?? 0)}</span>
								<span>{formatMs(progressDurationMs ?? 0)}</span>
							</div>
						</div>
					{/if}
					...
					<div class="now-controls">
						{#if useBridge}
							<button
								type="button"
								class="now-btn-small"
								class:on={spotifyPlayerState.shuffle}
								onclick={() => void spotifyControl('shuffle', { enabled: !spotifyPlayerState.shuffle })}
								aria-label="Shuffle"
							>
						...
						{:else if useMaHub && !maIsRadio}
							<button
								...
								class="now-btn-small"
								class:on={maShuffle}
								onclick={() => void spotifyControl('shuffle', { enabled: !maShuffle })}
								aria-label="Shuffle"
							>
								<StatusIcon icon="mdi:shuffle" size={16} />
							</button>
						...
						{#if !(useMaHub && maIsRadio)}
							<button
								type="button"
								class="now-btn"
								disabled={!on || actionBusyEntityId === activeEntity.entityId}
								onclick={() =>
									useBridge || useMaHub ? void spotifyControl('previous') : void prevTrack(activeEntity)}
								aria-label="Vorige"
							>
						...
						{#if !(useMaHub && maIsRadio)}
							<button
								...
								disabled={!on || actionBusyEntityId === activeEntity.entityId}
								onclick={() =>
									useBridge || useMaHub ? void spotifyControl('next') : void nextTrack(activeEntity)}
								aria-label="Volgende"
							>
						...
						{#if useBridge}
							<button
								type="button"
								class="now-btn-small"
								class:on={spotifyPlayerState.repeat !== 'off'}
								onclick={() => {
									const nextMode =
										spotifyPlayerState.repeat === 'off'
											? 'context'
											: spotifyPlayerState.repeat === 'context'
												? 'track'
												: 'off';
							...
							>
								<StatusIcon
									icon={spotifyPlayerState.repeat === 'track' ? 'mdi:repeat-once' : 'mdi:repeat'}
									size={16}
								/>
							</button>
							<button
								...
								class="now-btn-small"
								onclick={() => {
									spotifyQueueOpen = !spotifyQueueOpen;
									if (spotifyQueueOpen) void loadSpotifyQueue();
								}}
								aria-label="Wachtrij"
							>
					...
					<div class="now-volume">
						<button
							...
							class="vol-mute"
							disabled={!on}
							onclick={() => void toggleMute(activeEntity)}
							aria-label="Mute"
						>
							<StatusIcon
								icon={muted || vol <= 0.005
									? 'mdi:volume-off'
									: vol < 0.34
										? 'mdi:volume-low'
										: vol < 0.67
						...
						<input
							...
							oninput={(e) =>
								onVolumeInput(Number((e.currentTarget as HTMLInputElement).value), activeEntity)}
						/>
					...
					{#if sources.length > 0}
						<div class="now-source">
							<button
								...
								onclick={() => (sourcePickerOpen = !sourcePickerOpen)}
							>
								...
								<div class="source-popover">
									{#each sources as source (source)}
										<button
											...
											onclick={() => void selectSource(activeEntity, source)}>{source}</button
										>
									{/each}
								</div>
							{/if}
	...
	<div class="section-tabs" role="tablist">
		...
		>
			<StatusIcon icon="mdi:speaker-multiple" size={16} />
			{_t('Spelers')}
		</button>
		...
		<button
			type="button"
			role="tab"
			class:active={activeSection === 'tunein'}
			onclick={() => (activeSection = 'tunein')}
		>
			...
			{#if !spotifyConnected}
				<div class="block-empty">
					...
					<button
						...
						onclick={spotifyConnectStart}
						disabled={spotifyAuthInProgress}
					>
						{spotifyAuthInProgress ? _t('Bezig met verbinden…') : _t('Verbinden met Spotify')}
					</button>
					{#if spotifyAuthInProgress}
						<p class="block-hint">
							{_t('Voltooi de Spotify-koppeling in het nieuwe tabblad. Deze pagina pikt het automatisch op.')}
						</p>
					{/if}
				</div>
			{:else}
				...
				<div class="spotify-row">
					<select
						class="spotify-target"
						value={spotifyPlaybackTarget}
						onchange={(e) => setSpotifyPlaybackTarget((e.currentTarget as HTMLSelectElement).value)}
					>
					...
					<button
						type="button"
						class="sp-btn"
						onclick={() => (bridgeManagerOpen = !bridgeManagerOpen)}
						aria-label="Onkyo zones beheren"
						title="Onkyo zones beheren"
					>
								...
								<div class="bridge-row">
									{#if bridgeRenamingId === b.id}
										...
										<button
											type="button"
											class="bridge-btn ok"
											onclick={commitBridgeRename}
											aria-label={_t('save')}><StatusIcon icon="mdi:check" size={14} /></button
										>
										<button
											type="button"
											class="bridge-btn"
											onclick={cancelBridgeRename}
											aria-label={_t('cancel')}><StatusIcon icon="mdi:close" size={14} /></button
										>
									{:else}
										...
										<button
											...
											onclick={() => startBridgeRename(b.id)}
											aria-label={_t('Hernoemen')}
											title={_t('Hernoemen')}><StatusIcon icon="mdi:pencil-outline" size={14} /></button
										>
										<button
											type="button"
											class="bridge-btn ghost"
											onclick={() => removeBridge(b.id)}
											aria-label={_t('Verwijderen')}
											title={_t('Verwijderen')}><StatusIcon icon="mdi:trash-can-outline" size={14} /></button
										>
									{/if}
								</div>
							{/each}
				...
				<div class="sp-search-row">
					<div class="sp-search-input-wrap">
						<input
							...
							onkeydown={(e) => {
								if (e.key === 'Enter') void spotifySearch();
							}}
						/>
					...
					<button
						type="button"
						class="sp-btn sp-btn-search"
						onclick={() => void spotifySearch()}
						disabled={spotifySearchBusy}
					>
				...
				{#if spotifySearchResults.length > 0}
					<div class="sp-section-header sp-section-header-row">
						...
						<button
							type="button"
							class="sp-section-close"
							onclick={() => {
								spotifySearchResults = [];
								spotifySearchQuery = '';
							}}
							aria-label="Resultaten sluiten"
						...
						{#each spotifySearchResults as r (r.uri)}
							<div class="sp-art-tile">
								<button
									type="button"
									class="sp-art-cover"
									onclick={() => void spotifyPlayUri(r.uri)}
									aria-label={`Afspelen: ${r.name}`}
								>
									{#if r.image}
										<img src={r.image} alt="" />
									{:else}
										<div class="sp-art-empty">
											<StatusIcon
												icon={r.kind === 'track'
													? 'mdi:music'
													: r.kind === 'album'
								...
								{#if r.kind === 'track'}
									<button
										...
										onclick={() => void spotifyAddToQueue(r.uri)}
									>
						...
						{#each spotifyPlaylists as p (p.uri)}
							<div class="sp-art-tile">
								<button
									type="button"
									class="sp-art-cover"
									onclick={() => void spotifyPlayUri(p.uri)}
									aria-label={`Afspelen: ${p.name}`}
								>
									{#if p.image}
										<img src={p.image} alt="" />
									{:else}
				...
				{#if spotifyError}
					<div class="sp-error">
						...
						<button
							type="button"
							class="sp-error-close"
							onclick={() => (spotifyError = '')}
							aria-label={_t('close')}
						>
							<StatusIcon icon="mdi:close" size={14} />
						</button>
		...
		<section class="hub-block">
			<header class="block-head">
				...
				<button
					type="button"
					class="block-add"
					onclick={() => (tuneInShowSearch = !tuneInShowSearch)}
					aria-label={_t('Zoeken')}
					title={_t('Stations zoeken')}
				>
				...
				<button
					type="button"
					class="block-add"
					onclick={openCustomRadioDialog}
					aria-label={_t('Eigen URL')}
					title={_t('Eigen stream-URL toevoegen')}
				>
			...
			{#if maAvailable}
				<div class="spotify-row">
					<select
						class="spotify-target"
						value={tuneInPlaybackTarget}
						onchange={(e) => setTuneInPlaybackTarget((e.currentTarget as HTMLSelectElement).value)}
					>
						<option value="">{_t('Kies speelapparaat…')}</option>
						<optgroup label="Speakers (Music Assistant)">
			...
			{#if tuneInShowSearch}
				<div class="tin-search-row">
					<input
						...
						onkeydown={(e) => {
							if (e.key === 'Enter') void searchTuneIn();
						}}
					/>
					<button
						type="button"
						class="tin-search-btn"
						onclick={() => void searchTuneIn()}
						disabled={tuneInSearchBusy}
					>
						<StatusIcon icon="mdi:magnify" size={18} />
					</button>
				</div>
							...
							<div class="tin-result">
								...
								<div class="tin-result-actions">
									<button
										...
										class="tin-mini-btn"
										title="Nu afspelen"
										onclick={() => void playTuneInItem(r)}
									>
										<StatusIcon icon="mdi:play" size={14} />
									</button>
									{#if isTuneInFavorite(r.url)}
										<span class="tin-saved" title="Al in favorieten"
											><StatusIcon icon="mdi:star" size={14} /></span
										>
									{:else}
										<button
											type="button"
											class="tin-mini-btn"
											title={_t('Aan favorieten toevoegen')}
											onclick={() => addTuneInFavorite(r)}
										>
					...
					{#each tuneInFavorites as fav (fav.id)}
						<div class="sp-art-tile">
							<button
								type="button"
								class="sp-art-cover"
								onclick={() => void playTuneInItem(fav)}
								aria-label={`Afspelen: ${fav.text}`}
							>
								{#if fav.image}
							...
							<button
								type="button"
								class="sp-art-remove"
								onclick={() => removeTuneInFavorite(fav.id)}
								aria-label={_t('Verwijderen')}
								title={_t('Verwijderen uit favorieten')}
							>
		...
		<section class="hub-block">
			...
			<div class="players-grid">
				{#each sortedEntities as entity (entity.entityId)}
					...
					{@const usesSpotifyState =
						isOnkyoZone &&
						spotifyPlayerState.track &&
						Boolean(spotifyPlayerState.device) &&
						/onkyo|tx[-_ ]?nr/i.test(spotifyPlayerState.device?.name ?? '')}
					{@const mediaEntity = usesSpotifyState ? entity : richestEntityFor(entity, entities, allEntities)}
					{@const playing = usesSpotifyState ? Boolean(spotifyPlayerState.isPlaying) : isPlaying(entity)}
					{@const rawBaseCover = usesSpotifyState
						? (spotifyPlayerState.track?.image ?? '')
						: readMediaImage(mediaEntity)}
					...
					{@const baseSummary = !on
						...
						: usesSpotifyState
							? {
									primary: spotifyPlayerState.track?.name ?? '',
									secondary: spotifyPlayerState.track?.artists ?? ''
								}
							: readNowPlayingSummary(mediaEntity)}
					{@const appName = (entity.attributes?.app_name as string | undefined) ?? ''}
					{@const nowPlaying =
						!baseSummary.primary && !baseSummary.secondary && on && appName
					...
					>
							...
							<div class="player-tile-cover">
								{#if cover}
									<img
										src={cover}
										alt=""
										class={isLogoCover ? 'is-logo' : ''}
										onerror={() => markDisplayImageFailed(cover)}
									/>
								{:else}
							...
							<div class="player-tile-info">
								...
								{#if on && nowPlaying.primary}
									<div class="player-tile-now">
										...
										{#if nowPlaying.secondary}<div class="player-tile-now-secondary">
						...
						<div class="player-tile-controls">
							<button
								type="button"
								class="ptile-btn"
								disabled={!on || actionBusyEntityId === entity.entityId}
								onclick={(event) => {
									event.stopPropagation();
									void prevTrack(entity);
								}}
								aria-label={_t('Vorige')}><StatusIcon icon="mdi:skip-previous" size={18} /></button
							>
							<button
								type="button"
								class="ptile-btn ptile-btn-play"
								disabled={actionBusyEntityId === entity.entityId}
								onclick={(event) => {
									event.stopPropagation();
									void togglePlayPause(entity);
								}}
								aria-label={playing ? _t('Pauzeren') : _t('Afspelen')}
								><StatusIcon
									icon={!on ? 'mdi:power' : playing ? 'mdi:pause' : 'mdi:play'}
									size={20}
								/></button
							>
							<button
...
{#if customRadioOpen}
	<button
		type="button"
		class="np-modal-overlay"
		onclick={() => (customRadioOpen = false)}
		aria-label={_t('close')}
	></button>
	<div
		class="np-modal"
		role="dialog"
		aria-modal="true"
		use:modalBehavior={{ onClose: () => (customRadioOpen = false) }}
	>
		...
		<label class="np-field">
			...
			<input
				...
				oninput={(e) => (customRadioName = (e.currentTarget as HTMLInputElement).value)}
				placeholder="Bijv. NPO Radio 1"
			/>
		</label>
		<label class="np-field">
			...
			<input
				...
				oninput={(e) => (customRadioUrl = (e.currentTarget as HTMLInputElement).value)}
				placeholder="https://icecast.omroep.nl/radio1-bb-mp3"
			/>
		</label>
		<div class="np-modal-actions">
			<button type="button" class="np-btn-secondary" onclick={() => (customRadioOpen = false)}
				>{_t('cancel')}</button
			>
			<button
				type="button"
				...
				disabled={!customRadioName.trim() || !customRadioUrl.trim()}>{_t('save')}</button
			>
		</div>
	</div>
{/if}
...
<style>
	...
	.now-bg-overlay {
		...
		background: linear-gradient(
			180deg,
			rgba(15, 20, 36, 0.45) 0%,
			rgba(15, 20, 36, 0.75) 70%,
			rgba(15, 20, 36, 0.92) 100%
		);
		pointer-events: none;
	}
	.now-bg-fallback {
		position: absolute;
		inset: 0;
		background: linear-gradient(135deg, rgba(96, 165, 250, 0.1), rgba(40, 50, 70, 0.5));
		pointer-events: none;
	}
	...
	.now-btn {
		...
		width: 46px;
		height: 46px;
		background: radial-gradient(circle at 30% 30%, rgba(255, 255, 255, 0.1), rgba(255, 255, 255, 0.04) 70%);
	...
	.now-btn-play {
		...
		color: #0f1424;
		box-shadow:
			0 12px 28px rgba(147, 197, 253, 0.4),
	...
	.spotify-connect-btn:hover:not(:disabled) {
		background: #1ed760;
	}
	.spotify-connect-btn:disabled {
		background: rgba(29, 185, 84, 0.4);
		cursor: default;
	}
	.block-hint {
		font-size: 0.78rem;
		color: rgba(143, 198, 255, 0.85);
		margin: 0;
	...
	.sp-info-block.sp-info-ok {
		color: rgba(180, 235, 180, 0.85);
		background: rgba(110, 196, 100, 0.08);
		border-color: rgba(110, 196, 100, 0.25);
	}
	.sp-wake {
	...
	.bridge-rename {
		...
		border: 1px solid rgba(79, 113, 168, 0.7);
		color: #f5f5f5;
		border-radius: 0.3rem;
	...
	.bridge-rename:focus-visible {
		box-shadow: 0 0 0 3px rgba(79, 113, 168, 0.28);
	}
	.bridge-btn {
		display: inline-flex;
		align-items: center;
	...
	.sp-search-input:focus-visible {
		box-shadow: 0 0 0 3px rgba(29, 185, 84, 0.24);
	}
	...
	.sp-art-empty {
		...
		align-items: center;
		justify-content: center;
		background: linear-gradient(135deg, rgba(96, 165, 250, 0.1), rgba(40, 50, 70, 0.4));
		color: rgba(255, 255, 255, 0.45);
	}
	.sp-art-empty.radio {
		background: linear-gradient(135deg, rgba(167, 139, 250, 0.1), rgba(40, 50, 70, 0.4));
	}
	...
	.sp-art-tile:hover .sp-art-queue {
		opacity: 1;
	}
	.sp-art-queue:hover {
	...
	.sp-art-tile:hover .sp-art-remove {
		opacity: 1;
	}
	.sp-art-remove:hover {
	...
	.sp-error {
		...
		color: #f3ca62;
		padding: 10px 12px;
		background: linear-gradient(135deg, rgba(243, 202, 98, 0.1), transparent 60%), rgba(243, 202, 98, 0.04);
	...
	.tin-search-input:focus {
		outline: none;
		border-color: rgba(79, 113, 168, 0.7);
	}
	.tin-search-input:focus-visible {
		box-shadow: 0 0 0 3px rgba(79, 113, 168, 0.28);
	}
	.tin-search-btn {
		width: 2.4rem;
	...
	.tin-search-btn:hover:not(:disabled) {
		background: rgba(255, 255, 255, 0.16);
	}
	.tin-results {
		display: flex;
	...
	.tin-mini-btn:hover:not(:disabled) {
		background: rgba(79, 113, 168, 0.4);
	}
	.tin-mini-btn:disabled {
		opacity: 0.4;
		cursor: default;
	}
	.tin-saved {
	...
	.player-tile.active {
		...
		box-shadow:
			0 0 0 0.5px rgba(96, 165, 250, 0.3),
	...
	.player-tile:hover .player-tile-bg {
		opacity: 0.55;
	}
	...
	.player-tile-bg-fallback {
		...
		inset: 0;
		background:
			linear-gradient(135deg, rgba(96, 165, 250, 0.05), transparent 60%), rgba(255, 255, 255, 0.015);
		pointer-events: none;
		z-index: 0;
	}
	...
	.player-tile-eq span:nth-child(1) {
		animation-delay: -0.5s;
	}
	.player-tile-eq span:nth-child(2) {
		animation-delay: -1s;
	}
	.player-tile-eq span:nth-child(3) {
		animation-delay: -0.2s;
	}
	.player-tile-eq span:nth-child(4) {
	...
	.ptile-btn-play :global(.mdi-mask) {
		--mask-color: #0f1424;
	}
