<script lang="ts">
	 ...
	 * getoond zolang `open` true is. Klikken op de achtergrond sluit zonder de
	 * draft op te slaan; klikken in de modal zelf doet niets dankzij stopPropagation.
	 */
		...
		>
			<div style="padding:0.85rem 1rem 0;display:flex;flex-direction:column;gap:0.3rem;">
				...
				<span style="font-size:0.75rem;opacity:0.5;"
					>{translate('Selecteer lampen voor deze groep', $selectedLanguageStore)}</span
				>
			</div>
			<div
				class="lg-picker-scroll"
				style="overflow-y:auto;scrollbar-width:none;padding:0.5rem 1rem;display:flex;flex-direction:column;gap:0.15rem;"
			>
				{#each statusEntityIds ?? [] as entityId (entityId)}
					{@const inOther = groups.some((g) => g.id !== editingId && g.entityIds.includes(entityId))}
					<label
						style="display:flex;align-items:center;gap:0.5rem;padding:0.45rem 0.5rem;border-radius:0.4rem;cursor:pointer;{inOther
							? 'opacity:0.4;cursor:not-allowed;'
							: ''}"
					>
						<input
							type="checkbox"
							checked={draftEntityIds.includes(entityId)}
							disabled={inOther}
							onchange={() => onToggleEntity(entityId)}
						/>
						<span style="flex:1;font-size:0.85rem;">{getFriendlyName(entityId)}</span>
						{#if inOther}<span style="font-size:0.68rem;opacity:0.5;"
								>{translate('in andere groep', $selectedLanguageStore)}</span
							>{/if}
					</label>
				{/each}
				{#if (statusEntityIds ?? []).length === 0}
					<div style="font-size:0.82rem;opacity:0.45;padding:1rem 0;text-align:center;">
						{translate('Voeg eerst lampen toe via de entiteitenlijst hierboven.', $selectedLanguageStore)}
					</div>
				{/if}
			</div>
