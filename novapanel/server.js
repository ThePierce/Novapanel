...
import { registerEnergyAssetRoutes } from './server/energy-asset-routes.js';
import {
	buildHaTargetUrl,
	haForwardPathFromTarget,
	isAllowedHaProxyForwardPath
} from './server/ha-proxy-paths.js';
import { registerMdiIconRoutes } from './server/mdi-icon-routes.js';
import {
	createSerializedTaskQueue,
	mergePanelStatePayload,
	panelStateWriteAck
} from './server/panel-state-store.js';
import { registerTuneInRoutes } from './server/tunein-routes.js';
import { asTrimmedString, splitCsv } from './server/shared.js';
...
const HA_PROXY_FETCH_TIMEOUT_MS = 15000;
const HA_CAMERA_FETCH_TIMEOUT_MS = 20000;
...
async function fetchWithTimeout(input, init = {}, timeoutMs = HA_PROXY_FETCH_TIMEOUT_MS) {
...
function cleanupSpotifyAuthStates(now = Date.now()) {
	for (const [stateToken, entry] of spotifyAuthStateMap.entries()) {
		const createdAt = entry && typeof entry === 'object' ? Number(entry.createdAt) : 0;
		if (!Number.isFinite(createdAt) || now - createdAt > SPOTIFY_AUTH_STATE_TTL_MS) {
			spotifyAuthStateMap.delete(stateToken);
		}
	}
}
...
const RESERVED_INGRESS_PREFIXES = new Set([
	'api',
	'local_novapanel',
	'_app',
	'favicon.ico',
...
function configuredCorsOrigins() {
	const values = [
		process.env.NOVAPANEL_ALLOWED_ORIGINS,
		process.env.NOVAPANEL_CORS_ALLOWED_ORIGINS,
		process.env.HASS_URL
	];
	return new Set(values.flatMap(splitCsv).map(normalizeOrigin).filter(Boolean));
}

function originMatchesRequestHost(origin, req) {
	try {
		const parsed = new URL(origin);
		const hostCandidates = [req.headers.host, req.headers['x-forwarded-host']]
			.flatMap((value) => String(value || '').split(','))
			.map((value) => value.trim().toLowerCase())
			.filter(Boolean);
		return hostCandidates.includes(parsed.host.toLowerCase());
	} catch {
		return false;
	}
}

function isDevelopmentLocalhostOrigin(origin) {
...
function isAllowedCorsOrigin(req, origin) {
	...
	if (originMatchesRequestHost(normalized, req)) return true;
	if (isDevelopmentLocalhostOrigin(normalized)) return true;
	return configuredCorsOrigins().has(normalized);
}

function normalizeIngressBasePath(value) {
	const trimmed = asTrimmedString(value).replace(/\/+$/, '');
	if (!trimmed) return '';
...
/** Lets trusted origins call panel-state / sync-config on the public HTTPS HA URL. */
...
const serializePanelStateWrite = createSerializedTaskQueue();

async function writePrivateJsonFile(filePath, payload) {
...
async function handlePanelStatePost(req, res) {
	...
	try {
		...
		const ack = await serializePanelStateWrite(async () => {
			let current = {};
			try {
				const raw = await fs.readFile(STATE_FILE, 'utf8');
				const parsed = JSON.parse(raw);
			...
			const merged = mergePanelStatePayload(current, body);
			await writePrivateJsonFile(STATE_FILE, merged);
			return panelStateWriteAck(merged);
		});
...
function extractRequestApiBase(req) {
	const proto = asTrimmedString(
		req.headers['x-forwarded-proto'] || req.headers['x-scheme'] || req.protocol || 'https'
	)
		.split(',')[0]
		.trim();
	const host = asTrimmedString(req.headers['x-forwarded-host'] || req.headers.host)
		.split(',')[0]
		.trim();
	const ingressPath = normalizeIngressBasePath(req.headers['x-ingress-path']);
	if (!host) return '';
...
async function writeSpotifyToken(tokenPayload) {
	try {
		await writePrivateJsonFile(SPOTIFY_TOKEN_FILE, tokenPayload);
		return true;
	} catch {
		return false;
	}
...
const handleHaConnection = async (req, res) => {
	...
	try {
		...
		const proxyTarget =
			typeof req.headers['x-proxy-target'] === 'string' ? req.headers['x-proxy-target'].trim() : '';
		const forwardedProto =
			typeof req.headers['x-forwarded-proto'] === 'string' ? req.headers['x-forwarded-proto'].trim() : '';
		...
		log(
			`ha connection requested: hassUrl=${hassUrl || '-'} internal=${internalHassUrl || '-'} browserToken=${token ? 'present' : 'missing'} proxyToken=${proxyAuth.token ? proxyAuth.source : 'missing'} envHassUrl=${envHassUrl ? 'present' : 'missing'} proxyTarget=${proxyTarget || '-'}`
		);
		res.status(200).json({
			hassUrl,
			token
...
app.get(
	[
		'/api/ha/connection',
		'/local_novapanel/api/ha/connection',
		'/api/hassio_ingress/:ingressToken/api/ha/connection'
	],
	handleHaConnection
);
...
function getHaWebSocketUrls(hassUrl) {
	const normalized = String(hassUrl || '')
		.trim()
		.replace(/\/+$/, '');
	...
	try {
		...
		if (basePath.endsWith('/core')) {
			return [`${origin}${basePath}/websocket`, `${origin}${basePath}/api/websocket`];
		}
...
async function callHaRestService(req, domain, service, serviceData = {}) {
	...
	const response = await fetchWithTimeout(
		`${hassUrl}/api/services/${safeDomain}/${safeService}`,
		{
			method: 'POST',
			headers: {
			...
			body: JSON.stringify(
				serviceData && typeof serviceData === 'object' && !Array.isArray(serviceData) ? serviceData : {}
			)
		},
...
async function callHaWebSocketOnce(socketUrl, token, payload, timeoutMs = 15000) {
	const resolvesWithFirstEvent =
		payload.type === 'calendar/event/subscribe' || payload.type === 'weather/subscribe_forecast';
...
async function handleHaStates(req, res) {
	...
	try {
		...
		if (!upstream.ok) {
			log(
				`ha states proxy GET /api/states -> ${upstream.status} target=${target.origin}${target.pathname} auth=${tokenSource}`
			);
		}
...
function setupHaWebSocketProxy(server) {
	...
	wsProxy.on('connection', async (client, req) => {
		if (activeConnectionCount >= HA_WS_MAX_CONNECTIONS) {
			client.close(1013, 'too_many_connections');
			return;
		}
		...
		try {
			...
			client.on('message', (data, isBinary) => {
				...
				if (!clientAuthenticated || !upstreamAuthenticated || upstream?.readyState !== WsClient.OPEN) {
					...
					if (
						queuedClientMessages.length >= HA_WS_MAX_QUEUED_MESSAGES ||
						queuedClientBytes + size > HA_WS_MAX_QUEUED_BYTES
					) {
						closeBoth(1009, 'queue_limit');
						return;
					}
					queuedClientBytes += size;
...
app.get(
	['/hacsfiles/*', '/local/community/advanced-camera-card/*', '/local/advanced-camera-card/*'],
	async (req, res) => {
		try {
			const { hassUrl, token } = await getHaProxyConfig(req);
			if (!hassUrl) {
...
async function proxyHaRequest(req, res) {
	try {
		...
		const timeoutMs = /\/api\/camera_proxy(?:_stream)?(?:\/|$)/.test(forwardUrl)
			? HA_CAMERA_FETCH_TIMEOUT_MS
			: HA_PROXY_FETCH_TIMEOUT_MS;
		const resolvedForwardPath = haForwardPathFromTarget(hassUrl, target);
		if (!isAllowedHaProxyForwardPath(resolvedForwardPath)) {
			log(
				`ha api proxy blocked ${method} ${resolvedForwardPath} target=${target.origin}${target.pathname} auth=${token ? tokenSource : 'none'}`
			);
			res.status(403).send('ha_api_proxy_path_not_allowed');
			return;
		}
		const upstream = await fetchWithTimeout(target, init, timeoutMs);
		if (!upstream.ok) {
			const cleanForwardPath = forwardUrl.split('?')[0] || '/';
			log(
				`ha api proxy ${method} ${cleanForwardPath} -> ${upstream.status} target=${target.origin}${target.pathname} auth=${token ? tokenSource : 'none'}`
			);
		}
		res.status(upstream.status);
		...
		const message = error instanceof Error ? error.message : String(error);
		log(`ha api proxy error: ${message}`);
		if (message === 'invalid_ha_forward_path') {
			res.status(403).send('ha_api_proxy_path_not_allowed');
			return;
		}
...
registerMdiIconRoutes(app, {
	fetchWithTimeout,
	log,
	timeoutMs: EXTERNAL_METADATA_FETCH_TIMEOUT_MS
});
...
app.post(spotifyPaths('/search'), async (req, res) => {
	try {
		...
		if (action === 'play') {
			...
			if (!response.ok) {
				await sendSpotifyUpstreamError(res, response, 'spotify_play_failed');
				return;
			}
...
async function handleSpotifyControl(req, res) {
	try {
		...
		if (!response.ok) {
			await sendSpotifyUpstreamError(res, response, 'spotify_control_failed');
			return;
		}
		res.status(200).json({ ok: true });
	} catch (error) {
...
async function handleSpotifyQueueAdd(req, res) {
	try {
		...
		const response = await spotifyApi(ensured.accessToken, `/me/player/queue?${params.toString()}`, {
			method: 'POST'
		});
		if (!response.ok) {
			await sendSpotifyUpstreamError(res, response, 'spotify_queue_failed');
			return;
		}
		res.status(200).json({ ok: true });
	} catch (error) {
...
registerTuneInRoutes(app, {
	fetchWithTimeout,
	log,
	timeoutMs: TUNEIN_FETCH_TIMEOUT_MS
});
